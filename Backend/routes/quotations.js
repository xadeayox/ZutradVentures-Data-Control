...
router.get('/', protect, quotationAccess, async (req, res) => {
    try {
        ...
        const shaped = quotations.map(q => ({
            ...
            mimeType: q.mimeType,
            clientFactory: q.client ? q.client.companyName : 'Unknown Client',
            uploadedBy: q.uploadedBy ? `${q.uploadedBy.firstName} ${q.uploadedBy.surname}` : 'Unknown User',
...
router.get('/download/:storedName', protect, quotationAccess, async (req, res) => {
    try {
        ...
        const quotation = await Quotation.findOne({ where: { storedName } });
