    ...
    try {
        const purchaseOrders = await PurchaseOrder.findAll({
            ...
            include: [
                ...
                { model: User, as: 'uploadedBy', attributes: ['id', 'firstName', 'surname'], required: false }
            ]
        });
        ...
        const shaped = purchaseOrders.map(po => ({
            ...
            clientFactory: po.client ? po.client.companyName : 'Unknown Client',
            uploadedBy: po.uploadedBy ? `${po.uploadedBy.firstName} ${po.uploadedBy.surname}` : 'Unknown User',
...
// ─── GET /api/purchase-orders/download/:storedName ─────────────────────────────
router.get('/download/:storedName', protect, poAccess, async (req, res) => {
    try {
        const { storedName } = req.params;
        const po = await PurchaseOrder.findOne({ where: { storedName } });
