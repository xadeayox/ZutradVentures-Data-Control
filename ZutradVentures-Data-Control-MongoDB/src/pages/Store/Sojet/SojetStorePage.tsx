...
export default function SojetStorePage({ searchTerm, setSearchTerm }: SearchTermProps) {
    ...
    async function fetchItems() {
        try {
            const response = await apiFetch(`/api/store/${encodeURIComponent('Sojet')}`);
            const data = await response.json();
            if (response.ok) setItems(data.items);
        } catch (err) {
            console.error('Failed to fetch Sojet store:', err);
        }
    }
