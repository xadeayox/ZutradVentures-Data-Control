...
export default function SavemaStorePage({ searchTerm, setSearchTerm }: SearchTermProps) {
    ...
    async function fetchItems() {
        try {
            const response = await apiFetch(`/api/store/${encodeURIComponent('Savema')}`);
            const data = await response.json();
            if (response.ok) setItems(data.items);
        } catch (err) {
            console.error('Failed to fetch Savema store:', err);
        }
    }
