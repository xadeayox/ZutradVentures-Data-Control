...
export default function MacsaStorePage({ searchTerm, setSearchTerm }: SearchTermProps) {
    ...
    async function fetchItems() {
        try {
            const response = await apiFetch(`/api/store/${encodeURIComponent('Macsa ID')}`);
            const data = await response.json();
            if (response.ok) setItems(data.items);
        } catch (err) {
            console.error('Failed to fetch Macsa store:', err);
        }
    }
